...
contextBridge.exposeInMainWorld('electron', {
  ...
  deleteLayout: (fileName: string) => ipcRenderer.invoke(IPC_CHANNELS.DELETE_LAYOUT, fileName),
  modifyLayout: (fileName: string, data: Partial<LayoutFile>) =>
  ...
  addOverlayToLayout: (layoutFileName: string, overlayFolderName: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.ADD_OVERLAY_TO_LAYOUT, layoutFileName, overlayFolderName),
  removeOverlayFromLayout: (layoutFileName: string, overlayId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.REMOVE_OVERLAY_FROM_LAYOUT, layoutFileName, overlayId),
  ...
  openOverlaysFolder: () => ipcRenderer.invoke(IPC_CHANNELS.OPEN_OVERLAYS_FOLDER),
  ...
  getGameDataShape: (game: GameName) => ipcRenderer.invoke(IPC_CHANNELS.GET_GAME_DATA_SHAPE, game),
...
contextBridge.exposeInMainWorld('actions', {
  minimize: () => ipcRenderer.invoke(IPC_CHANNELS.TITLE_BAR_MESSAGE, 'minimize'),
...
contextBridge.exposeInMainWorld('steam', {
  fetchWorkshopItems: (page: number, queryConfig?: WorkshopItemQueryConfig) =>
    ipcRenderer.invoke(IPC_CHANNELS.STEAM_GET_ALL_WORKSHOP_ITEMS, page, queryConfig),
