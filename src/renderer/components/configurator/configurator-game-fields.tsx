...
interface GameFieldsProps {
  ...
  setManifestData: (data: OverlayManifest | ((prev: OverlayManifest) => OverlayManifest)) => void;
...
export const ConfiguratorGameFields: React.FC<GameFieldsProps> = ({
  ...
  const addFieldToList = (field: string, listType: 'required' | 'optional') => {
    const listKey = listType === 'required' ? 'requiredFields' : 'optionalFields';
    const currentList = manifestData[listKey];
    const otherListKey = listType === 'required' ? 'optionalFields' : 'requiredFields';
  ...
  const removeFieldFromList = (field: string, listType: 'required' | 'optional') => {
    const listKey = listType === 'required' ? 'requiredFields' : 'optionalFields';
    setManifestData(prev => ({
      ...prev,
  ...
  const moveFieldBetweenLists = (field: string, fromList: 'required' | 'optional') => {
  ...
  const isFieldInAnyList = (field: string) => {
    return (
      manifestData.requiredFields.includes(field) || manifestData.optionalFields.includes(field)
    );
  };
          ...
          <div>
            ...
            >
              ...
              <SelectContent>
                {games.map(game => {
                  return (
                    <SelectItem key={`game-fields-selected-${game}`} value={game}>
                      {game}
                    </SelectItem>
                  );
                })}
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center justify-between">
                Required Fields
                <Badge variant="destructive">{manifestData.requiredFields.length}</Badge>
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-2">
              {manifestData.requiredFields.map(field => (
                <div key={field} className="bg-muted flex items-center justify-between rounded p-2">
                  <span className="min-w-0 flex-1 truncate font-mono text-sm">{field}</span>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center justify-between">
                Optional Fields
                <Badge variant="secondary">{manifestData.optionalFields.length}</Badge>
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-2">
