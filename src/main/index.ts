...
import { createModsFolder, getModFolderNames } from "./utils/modsFolder";
...
app.on("ready", () => {
  ...
  const modFolders = getModFolderNames();

  for (let i = 0; i < modFolders.length; i++) {
    windows.push(
      createOverlayWindow(path.join(MODS_PATH, modFolders[i], "index.html"), {
        width: 500,
        height: 200,
      })
    );
