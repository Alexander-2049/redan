...
function generateName(i: number): [string, string, string] {
  ...
  return [first[i % first.length], last[i % last.length], middle[i % middle.length]];
}
...
function calculateLapDistTotalPct(driverIndex: number, frameIndex: number): number {
...
function generateDriver(driverIndex: number, frameIndex: number): iRacingDriverData {
